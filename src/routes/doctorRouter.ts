...
doctorRouter.get("/specialisation/:_id", async (req, res) => {
  try {
    ...
    console.log("specializare Lets go");
    console.log(_id);
    ...
    if (!specialisation) {
      return res
        .status(404)
        .send({ success: false, message: "Specialisation not found" });
    }

    const doctors = await User.find({
      role: UserRoles.Doctor,
      specialisation: specialisation._id,
    });
    ...
    return res
      .status(200)
      .send({ success: true, message: "Doctors found", doctors });
  } catch (err) {
