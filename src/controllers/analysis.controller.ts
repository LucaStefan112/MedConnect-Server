...
export const getAnalyses = async (req: Request, res: Response) => {
  console.log("getAnalyses Accesat");
  ...
  return res
    .status(200)
    .send({ success: true, message: "analyses-found", analyses });
};
...
export const getAnalysis = async (req: Request, res: Response) => {
  ...
  if (!analysis) {
    return res
      .status(404)
      .send({ success: false, message: "Analysis not found" });
  }
  ...
  return res
    .status(200)
    .send({ success: true, message: "analysis-found", analysis });
};
